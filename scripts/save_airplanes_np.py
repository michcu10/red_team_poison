...
def main():
    out_dir = 'artifacts/airplane_samples'
    os.makedirs(out_dir, exist_ok=True)
    
    # Load test batch to get varied airplanes
    batch = load_cifar_batch('data/cifar-10-batches-py/test_batch')
    data = batch[b'data']
    ...
    # Find airplane images (label 0)
    airplane_indices = [i for i, label in enumerate(labels) if label == 0]
    
    for i in range(3):
        idx = airplane_indices[i]
        img = data[idx]
        
        # Save original
        orig_pil = Image.fromarray((img.transpose(1, 2, 0) * 255).astype(np.uint8))
        orig_pil.save(f'{out_dir}/airplane_{i}_original.png')
        
        # Save patch poisoned
        patch_img = add_patch_trigger_np(img)
        patch_pil = Image.fromarray((patch_img.transpose(1, 2, 0) * 255).astype(np.uint8))
        patch_pil.save(f'{out_dir}/airplane_{i}_patch.png')
        ...
        freq_pil.save(f'{out_dir}/airplane_{i}_freq.png')
        
        print(f"Generated samples for airplane {i}")
